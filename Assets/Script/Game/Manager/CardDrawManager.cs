...
/// <summary>
/// 덱에서 카드를 뽑는 '드로우 애니메이션'을 담당합니다. (2D UI 환경)
/// 덱 UI 위치에서 카드가 생성되어 -> 화면 중앙 UI에 잠시 보였다가 -> 손패로 들어가는 연출을 합니다.
/// </summary>
public class CardDrawManager : MonoBehaviour
{
    ...
    [Header("프리팹 및 씬 연결")]
    public GameObject cardPrefab; // UI 카드 프리팹

    // [변경점 1] 3D Transform 대신 UI용 RectTransform 사용
    public RectTransform deckTransform; // 덱 UI 위치
    [Tooltip("카드를 뽑았을 때 잠시 보여줄 UI 위치 (보통 화면 중앙)")]
    public RectTransform showCardTransform;

    [Header("핵심 연결")]
    // [변경점 2] 새로 만든 2D UI 전용 손패 매니저로 교체
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.S)) // 단일 드로우 테스트
        {
            var testCardData = new CardInfo
            {
    ...
    /// <summary>
    /// [핵심] 카드 ID를 이용해 실제 데이터를 찾습니다. (기존 로직 유지)
    /// </summary>
    public CardData GetCardDataById(string id)
    {
        ...
        // 2. Firebase(서버)에서 최신 밸런스 데이터 확인
    ...
    {
        if (cardPrefab == null || deckTransform == null || showCardTransform == null || handCardControllManager == null) return;

        Quaternion flippedRotation = deckTransform.rotation * Quaternion.Euler(0, 180f, 0);

        // 1. 덱 위치에 카드 생성 
        // [변경점 3] 생성 시 deckTransform의 부모(Canvas 관련)를 부모로 지정하여 UI에서 정상적으로 렌더링되게 함
        ...
        // [변경점 4] 뽑히는 카드가 다른 UI나 기존 카드들에 가려지지 않도록 하이어라키 맨 아래(화면 맨 앞)로 보냄
        ...
        }else if(GameEntityManager.Instance.test)
        {
        ...
        // 180도 뒤집힌 상태에서 -> showCardTransform의 회전값(앞면)으로 보간되면서 카드가 휙 돌아갑니다.
        drawSequence.Join(
        ...
        drawSequence.OnComplete(() =>
        {
            ...
            // [변경점 5] 새로운 HandCardControllManager의 AddCardToHand 호출
